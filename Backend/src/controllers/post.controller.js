...
const getPaginatedPosts = async (req, res) => {
	try {
		...
		console.log(error);
		return res.status(500).json({ message: 'Failed to get paginated posts' });
	}
};

const getPaginatedUserPosts = async (req, res) => {
	try {
		const page = Math.max(Number.parseInt(req.query.page, 10) || 1, 1);
		const limit = 10;
		const skip = (page - 1) * limit;
		...
		return res
			.status(500)
			.json({ message: 'Failed to get paginated user posts' });
	}
};
...
const createComment = async (req, res) => {
	try {
		...
		if (!text) {
			return res.status(400).json({ message: 'Empty comments are Invalid' });
		}
