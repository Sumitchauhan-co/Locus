	...
	return (
		<div className="relative group">
			<motion.div
                ...
                h-fit w-full relative p-2 flex flex-col border border-(--border-color) rounded-xl ${isOwner ? 'rounded-tl-none' : ''} bg-(--tertiary-color) hover:bg-(--post-bg-color) transition-colors ${showComment ? 'rounded-b-none' : ''}`}
			>
				<div className="flex flex-col relative mb-2 gap-3">
					...
					<p className="sm:text-[0.925rem] text-[1rem]">{post.caption}</p>
				...
				<div className="flex items-center gap-10 px-1 mt-1">
					...
					>
						...
						<span className="text-sm">{post.likesCount?.length || 0}</span>
					</motion.button>
					<button
			...
			{showComment && (
				<Comment
					className={`w-full h-[600px] sm:h-[550px] `}
