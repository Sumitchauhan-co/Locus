...
const text =
	'The central point where your moments meet the map. Pin your life to the world and discover the social pulse of every place.';
...
const Introduction: React.FC = () => {
	...
	return (
		<motion.section
			...
			className="h-[300vh] sm:py-25 sm:px-5 px-2"
		>
			<div className="sticky h-screen top-0 overflow-hidden flex flex-col justify-center items-center">
				<motion.div
					style={{ x: translateX }}
					className="text-center text-5xl sm:text-7xl leading-relaxed font-medium whitespace-nowrap "
				>
					{words.map((word, i) => (
						<span
							key={i}
							className={`transition duration-300 font-[sans-serif] tracking-[0.25rem] italic ${
								i <= currentWord
