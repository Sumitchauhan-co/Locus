...
export default function Loader({ onFinish, isAuthLoading }: LoaderProps) {
	...
	return (
		<div className="relative h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-black select-none touch-none font-sans">
			{/* COUNTER: HEARTS BROKEN */}
			<motion.div
				initial={{ opacity: 0, y: -10 }}
				animate={{ opacity: 1, y: 0 }}
				className="absolute top-6 right-6 z-30 flex items-center gap-2 border border-pink-500/30 bg-pink-500/10 backdrop-blur-md px-4 py-2 rounded-full shadow-[0_0_15px_rgba(236,72,153,0.2)] pointer-events-none"
			>
				<HeartSVG className="w-5 h-5 text-pink-400" />
				<span className="text-pink-300 font-medium text-sm sm:text-base">
					Sparked: <span className="font-bold text-white">{brokenCount}</span>
				</span>
			</motion.div>
					...
					>
						...
						>
							...
							{GRID_TILES.map((tile) => (
								<div
									key={tile.id}
									className={`relative aspect-square ${tile.column >= COLS_MOBILE ? 'hidden sm:block' : 'block'}`}
								>
									{tile.isVisible && (
										<motion.div
											custom={tile.delay}
