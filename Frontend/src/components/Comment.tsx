...
const Comment: React.FC<CommentProps> = ({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (newComment && newComment._id) {
				...
				console.error('New comment data missing from response', res.data);
			}
		} catch (err) {
	...
	return (
		<section
			className={twMerge(
				'h-full z-1 flex flex-col bottom-0 bg-(--primary-color) border border-(--border-color) rounded-b-xl',
				className,
			)}
		>
			{showComment && (
				<div className="h-[450px] sm:h-[400px] w-full">
					<div className="text-2xl sm:text-xl p-3 flex justify-between">
						<h3>Comments</h3>
						<div
					...
					<div className="h-[400px] sm:h-[350px] w-full overflow-y-scroll border-y border-(--border-color)">
						{!loading &&
							comments.map((comment) => {
								const isCommentOwner =
									comment?.userId === currentUserId ||
									comment?.authId === currentUserId;
								const isMenuOpen = activeMenuId === comment?._id;

								const initial = comment?.username?.charAt(0).toUpperCase();
								const bgColor = getBgColor(comment?.username || '');
										...
										<div className="flex flex-col relative gap-2">
											...
											{(isCommentOwner || isAdmin) && (
												<button
													...
													title="options"
													onClick={() =>
														setActiveMenuId(isMenuOpen ? null : comment._id)
													}
			...
			<form
				...
				className="h-[150px] flex justify-center items-center m-3"
			>
				<div className="w-full flex flex-col gap-5 items-center sm:mb-3">
					<input
						value={text}
						onChange={(e) => setText(e.target.value)}
						className="w-full outline-none p-2 text-lg sm:text-sm border border-(--border-color) rounded-xl sm:rounded-lg"
						type="text"
						title="input"
